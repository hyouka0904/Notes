...
# Cloud DBMS: An Overview（雲端資料庫管理系統概述）

## Definition and Requirements（定義與需求）

### 定義（Definition）

- **雲端資料庫管理系統（Cloud DBMS）**：專為在雲端環境中運行而設計的資料庫管理系統
  - 可運行在實體機器（Physical machines）或虛擬機器（Virtual machines）上
  - 特別管理大量應用程式的資料，這些應用程式稱為**租戶（Tenants）**
  - 又稱為**多租戶資料庫管理系統（Multi-tenant DBMS）**

- **與傳統資料庫的區別**：
  - 僅在雲端實例上運行MySQL並不等於雲端資料庫
  - 必須從設計層面考慮雲端特性

### 核心需求：SAE特性（SAE Characteristics）

#### 高可擴展性（High Scalability）
- **水平擴展（Horizontal scaling）**：使用商品化硬體（Commodity machines）
- **高最大吞吐量（High maximum throughput）**：以每秒/每分鐘的交易數量或查詢數量來衡量
- **目標**：能夠處理大規模的並發請求

#### 高可用性（High Availability）
最可靠的是電話的網路。
通常雲服務商會保證三個九的保證度，大約一年會斷線個一個小時左右。
現在的亞馬遜可靠度是很高的。
- **持續運行**：即使面臨機器、網路或資料中心故障，系統仍能保持運行
- **故障容忍性**：系統設計必須考慮各種故障情況
- **服務不中斷**：確保使用者始終能夠存取服務

#### 彈性（Elasticity）
之所以會把東西放到雲端上是因為不知道專案最後會有多大
所以放到雲端上有多大用多大
動態分配機器來減少營運成本
這件事情就是所謂的彈性
- **動態資源調整**：根據當前工作負載動態新增或關閉機器
- **資料重新分配**：即時重新分配資料以最佳化效能
- **成本最佳化**：透過彈性調整來節省成本

### 傳統資料庫功能（Traditional DB Functions）

#### 表達式關聯式資料模型（Expressive Relational Data Model）
- 幾乎所有類型的資料都可以結構化為表格集合
- 提供靈活的查詢能力

#### 複雜但快速的查詢（Complex but Fast Queries）
- **跨表查詢**：支援多表聯結、分組、巢狀等操作
- **查詢最佳化**：查詢計畫最佳化、索引等機制
- **效能保證**：確保查詢執行效率

#### ACID交易（Transactions with ACID）
- **原子性（Atomicity）**：交易要麼全部執行成功，要麼全部回滾
- **一致性（Consistency）**：資料始終保持正確和持久
- **隔離性（Isolation）**：並發交易不會互相干擾
- **持久性（Durability）**：已提交的交易結果永久保存

### 功能重要性分析（Feature Importance Analysis）
如果你今天要設計一個cloud database基本的需求就在這裡
| 功能特性   | 重要性說明                                          |
| ---------- | --------------------------------------------------- |
| 關聯式模型 | 模擬幾乎所有資料類型，提供靈活查詢                  |
| 短延遲     | Google研究顯示100ms額外回應時間會導致使用者大量流失 |
| 交易與ACID | 確保資料正確性和持久性                              |
| 可擴展性   | 應對業務增長需求                                    |
| 可用性     | 無可用性就無服務                                    |
| 彈性       | 節省成本                                            |

## S through Partitioning（透過分割實現可擴展性）

### OLTP工作負載的可擴展性（Scalability for OLTP Workloads）
一個query最慢最面就是慢在buffer manager
另外的會就是latching
基本上buffer, latching, locking就佔據了大多數query的延遲

#### 吞吐量公式（Throughput Formula）
這個基本上完全depends on你的application
user再看商品的時候不是平均看
可能會有兩成的東西一直被重複看
如果你今天只是看報表之類的 conflict rate就會很低
$$\text{Max tx throughput} \propto \frac{1}{\text{contention footprint} \times \text{tx conflict rate}}$$

- **競爭足跡（Contention footprint）**：交易持有鎖的時間長度
  兩個contention conflict的時間會有多久
  有可能一個時間很短 那就不會有太大影響
  但也有可能時間很長然後又卡到其他很多交易
- **交易衝突率（Transaction conflict rate）**：交易之間的衝突頻率
- **瓶頸分析**：長時間持有鎖的交易會阻塞所有衝突的交易，降低吞吐量

### 分割策略（Partitioning Strategy）

#### 熱表分割（Hot Table Partitioning）
- **水平分割（Horizontal partitioning）**：將資料列分散到不同伺服器
- **垂直分割（Vertical partitioning）**：將資料行分散到不同伺服器
- **負載分散**：將讀寫負載分散到不同伺服器

#### 分散式系統複雜性（Distributed System Complications）

##### 分散式元資料管理（Distributed Metadata Manager）
- 管理跨多個節點的元資料資訊
- 確保元資料的一致性和可用性

##### 分散式查詢處理器（Distributed Query Processor）
- **全域最佳計畫（Global optimal plan）**：確定最佳的全域執行計畫
- **本地計畫（Local plans）**：將全域計畫分解為各節點的本地計畫

##### 分散式交易（Distributed Transactions）
- **全域交易ACID**：確保由多個本地交易組成的全域交易滿足ACID特性
- **本地交易協調**：協調各個本地交易的執行

### 隔離性重訪（Isolation Revisited）

#### 分散式並發控制管理器（Distributed Concurrency Control Manager）

##### 兩階段鎖定（Two-Phase Locking, 2PL）
- **專用鎖伺服器（Dedicated lock server）**：集中管理所有鎖
- **分散式嚴格兩階段鎖定（Distributed S2PL）**：每個鎖物件由主要伺服器管理
  這個很慢
  因為鎖占用的問題
  還要考慮deadlock的問題
  

##### 時間戳記與樂觀並發控制（Timestamp and Optimistic CC）
- **全域唯一時間戳記生成**：使用"local_counter@server_ID"格式
- **同步機制**：防止單一伺服器計數過快，每個伺服器在收到其他時間戳記時遞增自己的計數器

#### 分散式S2PL實現（Distributed S2PL Implementation）
- **主要伺服器（Primary server）**：每個物件的主要伺服器是擁有對應分割的機器
- **分散式死鎖問題**：需要檢測和解決跨節點的死鎖情況

### 原子性重訪（Atomicity Revisited）

#### 分散式交易提交（Distributed Transaction Commit）
- **全域交易提交**：提交全域交易意味著提交所有本地交易
- **回滾條件**：任何本地交易回滾都會導致全域交易回滾

#### 回滾原因（Rollback Causes）
- **ACID違反**：樂觀並發控制中的驗證失敗
- **死鎖**：交易間的循環等待
- **節點故障**：由副本等其他節點檢測到的節點故障

#### 提交協定（Commit Protocols）

##### 一階段提交（One-Phase Commit）
比如說提交兩台機器 但其中一台突然壞掉 造成部分提交
但我們要保證原子性
- **問題**：如果T2回滾，則T部分執行，違反原子性
- **後果**：T1和T3的效果由於持久性無法撤銷

##### 兩階段提交（Two-Phase Commit, 2PC）
- **階段1**：協調器詢問所有參與者是否可以提交
- **階段2**：根據所有參與者的回應決定提交或回滾
- **缺點**：長延遲，交易阻塞所有衝突交易，降低吞吐量
- **適用性**：只有當通訊開銷小於慢I/O開銷時，分割才有幫助

## A through Replication（透過複製實現可用性）

### 可用性策略（Availability Strategy）

#### 複製所有表（Replicate All Tables）
- **跨伺服器複製**：在多個伺服器上維護資料副本
- **地理分散**：理想情況下跨地理位置分離的區域以應對災難
- **備用副本**：如果一個區域的伺服器故障，仍有備用副本可用

### 一致性重訪（Consistency Revisited）
什麼時候?
誰來發起?
#### 分散式環境中的一致性定義
- **傳統一致性**：交易不會導致設定規則的違反
- **分散式一致性**：執行交易後所有副本保持相同狀態

#### R1WA協定（Read One Write All Protocol）
- **讀取策略**：交易本地讀取
- **寫入策略**：寫入所有副本
- **附加效益**：唯讀交易可以在任何副本上執行

#### 變更傳播（Change Propagation）
- **時機選擇**：積極（Eager）vs 懶惰（Lazy）
- **責任歸屬**：主從複製（Master/Slave）vs 多主複製（Multi-Master）

### 複製策略比較（Replication Strategy Comparison）

#### 積極複製（Eager Replication）
隨時隨地確保兩邊都一樣
- **同步寫入**：每個寫入操作必須在所有副本上完成後才能提交交易
- **2PC需求**：需要兩階段提交協定
- **故障處理**：任何副本故障都會導致交易回滾
- **特性**：交易慢但一致性強

#### 懶惰複製（Lazy Replication）
在交易提交之後
先提交給使用者 才在背後慢慢資料傳給遠端
現實大部分都是這個方案 沒有一致性就直接算了
因為失去一致性只有在機器出現故障的時候
好處就是快很多 對這個交易來說它就是本地的交易
對面要複製就看log就可以恢復
- **異步寫入**：寫入在本地完成，但在背景中傳播到遠端副本
- **日誌傳送**：通常透過傳送批次日誌實現
- **特性**：交易快但最終一致性

#### 主從 vs 多主複製（Master/Slave vs Multi-Master）
每一個紀錄都會有一個默認的老大
寫的時候你就要選要連到哪一個機器

##### 主從複製（Master/Slave Replication）
- **寫入路由**：記錄的寫入路由到特定（主）副本
- **讀取路由**：讀取可以從其他（從）副本進行

##### 多主複製（Multi-Master Replication）
很少有資料會在很短的時間內會被不同的機器同時被改的話
可以用 因為表現會更好
- **寫入分散**：記錄的寫入可以路由到任何副本
- **衝突處理**：同一記錄的兩次寫入可能由不同副本處理

### 複製策略評分表（Replication Strategy Scorecard）
...
- **積極主從與多主**：在2PL + 2PC下差異不大
- **懶惰多主**：需要衝突寫入的協調，可透過使用者或規則（如最後寫入勝出）處理

## Problems of Traditional DDBMS（傳統分散式資料庫管理系統的問題）

### 現實世界的DDBMS架構（Real-World DDBMS Architecture）

#### 架構概述（Architecture Overview）

這是一個典型的**懶惰主從複製（Lazy Master/Slave Replication）**架構，主導了傳統的分散式資料庫管理系統。
...
└─────────────┘    └─────────────┘    └─────────────┘
    ...
    │         ▼     ▼         ▼             ▼      ▼         ▼│
    │  ┌────────┐┌────────┐┌────────┐┌────────┐┌────────┐┌──────┐
    │  │DB Slave││DB Slave││DB Slave││DB Slave││DB Slave││......│
    │  │ (從庫)  ││ (從庫)  ││ (從庫)  ││ (從庫)  ││ (從庫)  ││      │
    │  └────────┘└────────┘└────────┘└────────┘└────────┘└──────┘
    └────────────────────────────────────────────────────────┘
                         │
...
#### 資料流向說明（Data Flow Description）

##### 讀寫交易流向（Read-Write Transaction Flow）
```
客戶端請求 → 調度器 → Web Server → DB Master (同步)
                                    │
                                    ▼
                            DB Slave (同步複製)
```

##### 唯讀交易流向（Read-Only Transaction Flow）
```
客戶端請求 → 調度器 → Web Server → DB Slave (直接讀取)
```

##### 跨資料中心複製流向（Cross-Datacenter Replication Flow）
```
資料中心A的DB Master → (異步日誌傳送) → 資料中心B的DB Master/Slave
```

#### 架構組件詳解（Component Details）
...
##### 讀取吞吐量擴展（Read Throughput Scaling）
```
複製策略: Master → Multiple Slaves (異步)
目的: 分散讀取負載
```

##### 寫入吞吐量擴展（Write Throughput Scaling）
```
分割策略: 資料水平分割到多個Master
目的: 分散寫入負載
```

##### 可用性保證（Availability Assurance）
```
跨資料中心複製: 資料中心A ↔ 資料中心B (異步)
目的: 災難恢復和地理分散
```

##### 短延遲最佳化（Low Latency Optimization）
```
本地化策略: 單資料中心內查詢
目的: 減少網路延遲
```

#### 架構限制與問題（Architecture Limitations）

##### 主要限制
1. **主庫瓶頸**：所有寫入操作集中在主庫
2. **一致性權衡**：讀取操作可能獲得過時資料
3. **故障時資料遺失**：異步複製可能導致資料遺失
4. **無彈性**：難以動態調整資源

##### 故障處理限制
- **資料中心故障**：只能提供唯讀服務或允許不一致性
- **主庫故障**：需要故障轉移時間，可能造成服務中斷
- **網路分割**：可能導致腦裂問題

#### 典型架構特點
- **讀寫交易**：強一致性（同步，單資料中心內）
- **唯讀交易**：弱一致性
- **主導模式**：懶惰主從複製主導傳統DDBMS

#### 架構組件
- **調度器（Dispatcher）**：負載平衡和請求路由
- **主資料庫（DB Master）**：處理寫入操作
- **從資料庫（DB Slave）**：處理讀取操作，具有熱備用節點監控心跳
- **跨資料中心複製**：異步複製以實現可用性和讀取吞吐量

### 核心問題分析（Core Problem Analysis）

#### 僅通過高階機器實現可擴展性（"S" Only with High-End Machines）

##### 分散式交易成本高昂（Distributed Transactions Are Costly）
- **資料中心內高延遲**：即使在單一資料中心內，2PC也會造成高延遲
- **主伺服器瓶頸**：每個讀寫交易都路由到主伺服器，形成潛在效能瓶頸

##### 解決方案限制（Solution Limitations）
- **精心資料分割 + 超強主伺服器**：
  - 盡可能減少分散式交易（特別是跨區域的）
  - 減少主伺服器數量
- **向上擴展成本**：向上擴展而非向外擴展 → 成本指數增長

#### 無故障情況：一致性與延遲的權衡（Without Failure: Trade-Off between C and Latency）

##### 懶惰主從複製權衡（Lazy M/S Replication Trade-offs）
- **一致性換取短延遲**：唯讀交易路由到從伺服器
- **不一致快照**：如果跨分割，讀取資料快照可能不一致
- **完全一致性支援**：允許客戶端要求唯讀交易通過主伺服器
  - 但需要更強大的主伺服器（更多成本）

#### 有故障情況：C與可用性權衡，且無持久性（With Failure: Trade-Off between C and Av, Plus No D）

##### 機器故障（Machine Failure）
- **熱備用伺服器**：每個主伺服器配備少數熱備用伺服器
- **就近放置**：放置在附近以減少開銷

##### 資料中心故障（Datacenter Failure）
- **故障原因**：電力短缺、災難等
- **權衡選擇**：
  - 故障轉移期間僅讀（無可用性）
  - 允許讀寫交易不一致性（無一致性）
- **資料遺失**：無論哪種方式，懶惰主從都會造成資料遺失

### DDBMS廠商現狀（DDBMS Vendor Status）

#### 廠商的妥協提案
> "如果你願意支付高額費用並犧牲一致性和持久性，我可以提供可擴展性和有限的可用性"
...
| 吞吐量             | 高（向上擴展）               |
| 瓶頸/SPF           | 主伺服器                     |
| 一致性（故障時）   | 寫入強一致性，讀取最終一致性 |
| 可用性（故障時）   | 僅讀                         |
| 資料遺失（故障時） | 一些                         |
| 衝突協調           | 不需要                       |
| 彈性               | 無                           |

## Usage Analysis: Operational vs. Analytic Workloads（使用分析：操作型與分析型工作負載）

### 資料庫工作負載分類（DB Workload Classification）

#### 操作型工作負載（Operational Workloads）
- **用途**：日常業務營運
- **別稱**：線上交易處理（OLTP, On-line Transaction Processing）
- **特點**：即時性要求高，交易頻繁

#### 分析型工作負載（Analytic Workloads）
- **用途**：資料分析和決策支援
- **形式**：線上（OLAP）或離線處理
- **特點**：複雜查詢，批次處理

### 工作負載特性矩陣（Workload Characteristics Matrix）
...
### 詳細特性比較（Detailed Characteristics Comparison）

| 特性       | 操作型                | 分析型                         |
| ---------- | --------------------- | ------------------------------ |
| 資料目的   | 最新快照              | 歷史或來自多個操作源的整合資料 |
| 資料量     | < 數十TB              | 數百TB到數百PB或更多           |
| 讀寫比例   | 60/40 到 98/2         | 主要是讀取                     |
| 查詢複雜度 | 短讀取，或短讀取+寫入 | 複雜讀取，批次寫入             |
| 延遲容忍度 | < 數秒                | 分鐘、小時、天或更多           |
| 交易+ACID  | 必需                  | 不太重要                       |

## The End of One-Size-Fits All Era（一體適用時代的終結）

### 傳統一體適用時代（Era of One-Size-Fits-All）

#### OLTP參與者（OLTP Players）
- **主要廠商**：Oracle（包括MySQL）、IBM DB2、PostgreSQL、MS SQL Server等
- **特點**：專注於交易處理和即時查詢

#### OLAP參與者（OLAP Players）
- **主要廠商**：IBM Infosphere、Teradata、Vertica、Oracle等
- **基礎架構**：基於星狀綱要（Star-schema）

### 當今挑戰（Today's Challenges）

#### SAE已成為必需（SAE is a Must）
- **彈性自動化**：E也意味著自動化能力
- **市場需求**：不再是可選功能，而是基本要求

#### 不滿意的OLTP使用者（Unhappy OLTP Users）
- **成本考量**：不願支付高額費用進行向上擴展
- **分割困難**：資料可能無法良好分割，即使有少數主伺服器
- **一致性需求**：在許多情況下需要一致性保證

#### 不滿意的分析型使用者（Unhappy Analytic Users）
- **網路規模資料**：資料量達到網路規模
- **資料格式問題**：資料格式不良且異質化
- **綱要變更**：綱要隨時間變化，缺乏固定結構
...
### 課程重點（Course Focus）
- **專注領域**：本課程重點關注OLTP系統
- **技術演進**：從傳統一體適用走向專業化解決方案

### 資料庫環境演進（The Evolving Database Landscape）

#### 新興趨勢
- **專業化系統**：不同工作負載需要不同的最佳化系統
- **混合架構**：結合多種技術以滿足不同需求
- **雲原生設計**：從零開始為雲環境設計的資料庫系統

#### 技術分類演進
- **關聯式**：傳統SQL資料庫持續演進
- **NoSQL**：鍵值、文件、圖形、列族等專門化存儲
- **NewSQL**：結合SQL語義與NoSQL可擴展性
- **雲服務**：Database-as-a-Service模式普及
