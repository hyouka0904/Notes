...
* 某些模型無法容忍測量相同基本量的預測變數（**多重共線性(multicollinearity )**或預測變數間相關性）
* 許多模型無法使用具有任何缺失值的樣本
...
## 11.4 逐步選擇（Stepwise Selection）

- **逐步選擇**（Stepwise Selection）最初是為線性迴歸模型（linear regression）開發的特徵選擇技術
  * 使用一系列步驟允許特徵一次一個地進入或離開迴歸模型
  * 這個程序通常會收斂到特徵的子集

- **進入與退出標準**：
  * 通常基於 p 值閾值（p-value threshold）
  * 典型的進入標準：p 值必須小於 0.15 才能進入模型
  * 典型的退出標準：p 值必須大於 0.15 才會離開模型

- **程序流程**：
  1. 建立 $p$ 個線性迴歸模型，每個模型恰好使用一個特徵
  2. 根據各特徵解釋結果變異的能力對其進行排名
  3. 解釋的變異量可簡化為 p 值
  4. 如果沒有特徵的 p 值小於 0.15，則停止程序
  5. 如果一個或多個特徵的 p 值小於 0.15，則保留 p 值最低的特徵
  6. 下一步建立 $p-1$ 個線性迴歸模型（包含第一步選擇的特徵和其他特徵）
  7. 評估每個額外特徵，加入符合納入標準的最佳特徵
  8. 計算每個特徵在其他特徵存在的情況下解釋的變異量
  9. 如果 p 值不超過排除標準，保留所有特徵並繼續尋找第三個特徵
  10. 如果特徵的 p 值超過排除標準，則從當前選定特徵集中移除（但可在稍後步驟重新進入）
  11. 繼續此過程直到滿足收斂標準

- **方法問題**：
  * 大量文獻對此方法提出批評（Steyerberg等人（1999）、Whittingham等人（2006）、Mundry和Nunn（2009））
  * Harrell（2015）全面抨擊此方法："如果這個程序剛被提出作為統計方法，它很可能會被拒絕，因為它違反了統計估計和假設檢驗的每一項原則"

- **逐步選擇的兩個主要缺陷**：
  1. **虛假陽性發現的膨脹**：
     * 使用許多重複的假設檢驗來決定個別預測變數的納入或排除
     * 相應的 p 值未經調整，導致特徵過度選擇（虛假陽性）
     * 當存在高度相關的預測變數時，此問題更加嚴重
  
  2. **模型過度擬合**：
     * 結果模型統計數據（包括參數估計和相關不確定性）高度樂觀
     * 因為它們沒有考慮選擇過程
     * 這個問題在本章和下一章描述的所有搜索方法中都存在
     * 個別模型統計數據因此不能被照字面理解

- **例外情況**：重採樣性能估計
  * 假設線性迴歸模型在 RFE 或全局搜索程序中使用
  * 內部估計的調整 $R^2$、RMSE 等將被樂觀估計
  * 但外部重採樣估計應更準確反映模型在新獨立數據上的預測性能
  * 這就是為什麼外部重採樣估計用於指導許多搜索方法

- **改進方法**：使用 AIC 而非 p 值
  * **赤池信息準則**（Akaike Information Criterion, AIC）是更好的選擇（Akaike 1974）
  * AIC 統計量專為使用似然函數（likelihood）作為目標的模型量身定制（如線性或邏輯迴歸）
  * 通過模型中包含的參數數量對似然函數進行懲罰
  * 因此，優化似然函數且參數較少的模型更受青睞
  * 操作上，在擬合初始模型後，可以計算每個包含新特徵或排除現有特徵的子模型的 AIC 統計量
  * 下一個模型對應於具有最佳 AIC 統計量的模型
  * 程序重複進行，直到當前模型包含最佳 AIC 統計量

- **AIC 計算公式**：
  * $AIC = -2\ell + 2p$
  * 其中 $\ell$ 是對數似然值，$p$ 是模型參數數
  * 目標是最小化 AIC 值

- **OkCupid 數據示例**：
  * 從包含年齡（age）、文章長度（essay length）和白種人指標（indicator for being Caucasian）的模型開始
  * 評估三個潛在特徵：nerd、firefly 和 im 關鍵詞的指標
  * 初始模型的二項式對數似然值為 -18525.3
  * 模型中有 4 個參數（每個項一個加一個截距）
  * AIC 計算：$AIC = -2(-18525.3) + 8 = 37059$
  * 逐步選擇的第一次迭代，通過刪除或添加變數創建六個模型並計算其 AIC 值
  * 結果按降序排列：
    - + nerd：36,863（最佳）
    - + firefly：36,994
    - + im：37,041
    ...
    - - age：37,080
    - - essay length：37,108
  * 基於這些值，添加任何三個關鍵詞都會改進模型，其中 nerd 產生最佳模型

- **作者建議**：
  * 完全避免使用這種程序
  * 正則化方法（如先前討論的 glmnet 模型）在線性模型中選擇適當子集方面要好得多
  * 如果需要模型推斷，可以使用多種貝葉斯方法（Mallick和Yi 2013；Piironen和Vehtari 2017b，2017a）

### 注意：
- 文中的 $p$ 是指預測變數的數量，不要與統計假設檢驗的 p 值混淆

## 11.5 總結（Summary）

- **特徵選擇目標**：
  * 識別能產生最佳模型的特徵子集通常是建模過程的目標
  * 當數據包含大量預測變數時，特徵選擇尤為重要
...
- **簡單方法 - 個別評估特徵**：
  ...
  * 這些值可轉換為 p 值，使不同類型間的比較成為可能
  * 為避免發現假陽性關聯，應在搜索中使用重採樣（resample）方法

- **簡單過濾器的優缺點**：
  * 優點：簡單過濾器（simple filters）非常適合找出個別預測變數
  * 缺點：不考慮多個特徵共同作用的影響
  * 可能導致選擇比實現最佳預測性能所需更多的特徵

- **遞迴特徵消除**（Recursive Feature Elimination, RFE）：
  * 可與任何模型結合以識別具有最佳性能的特徵子集
  * 優點：能識別良好的特徵子集，優化目標模型的性能
  * 主要缺點：要求初始模型能夠擬合整個預測變數集

- **逐步選擇方法**（Stepwise Selection）：
  * 一直是流行的特徵選擇技術
  * 缺點：
    - 使用 p 值作為添加或移除特徵的標準時有一些眾所周知的缺點
    - 應使用不具有與 p 值相同問題的替代指標（如 AIC）
    - 不能與許多現代預測模型一起使用

- **總體評價**：
  * 每種方法都有其適用場景和限制
  * 選擇適當的特徵選擇方法需考慮資料特性、模型類型和計算資源
  * 在真實應用中，可能需要結合多種方法以獲得最佳結果

# 12 全局搜索方法（Global Search Methods）

- 如第 10 章所述，**全局搜索方法**（global search methods）會調查多樣化的潛在解決方案集
  * 本章將討論兩種方法用於選擇適當的特徵子集：
    - **遺傳演算法**（genetic algorithms）
    - **模擬退火**（simulated annealing）
  * 其他可用的全局搜索方法包括：
    - **粒子群優化**（particle swarm optimization）
    - **同步擾動隨機近似**（simultaneous perturbation stochastic approximation）
  * 全面資源：Spall（2005）和 Weise（2011）這些優化技術的相關著作

- 在繼續之前，將先討論**單純貝葉斯分類模型**（naive Bayes classification model）
  * 這個分類器將貫穿本章，用於展示全局搜索方法

## 12.1 單純貝葉斯模型（Naive Bayes Models）

- 本章將使用 **OkCupid 數據**（OkCupid data）和**單純貝葉斯分類模型**（naive Bayes classification model）來說明方法
  * 此模型使用**貝葉斯公式**（Bayes formula）預測分類概率：

$$P[Class|Predictors] = \frac{P[Class] \times P[Predictors|Class]}{P[Predictors]} = \frac{Prior \times Likelihood}{Evidence}$$

- 這個公式的三個要素：
  1. **先驗概率**（Prior）：每個類別的一般概率（例如 STEM 簡歷的比率）
     * 可從訓練數據估計或使用專家知識確定
  2. **似然度**（Likelihood）：每個類別中觀察到的預測變數數據出現的相對概率
  3. **證據**（Evidence）：可從先驗和似然度計算出的標準化因子

- **單純貝葉斯的"單純"假設**：
  * 預測變數被假設為彼此**獨立**（independent）
  * 這使得聯合似然度可以計算為各個類別特定值的乘積
  * 實際應用中這個假設往往不成立，但模型仍可能產生良好預測

- **OkCupid 數據示例**：
  * 假設有兩個預測變數：散文中標點符號數量和宗教信仰
  * 對於**宗教信仰**（religion）預測變數（10個可能值）：
    - 建立預測變數值與結果之間的交叉表
    - 計算每個類別中每個宗教值的概率
    - 圖 12.1(a) 顯示類別特定概率
    - 例如：無神論者在 STEM 簡歷中概率為 0.213，其他為 0.103
  * 對於連續預測變數**標點符號數量**（number of punctuation marks）：
    - 分別計算每個類別的預測變數分布
    - 可通過分組預測變數並使用直方圖頻率估計概率
    - 更好的方法是計算這些數據的**非參數密度函數**（nonparametric density function）
    - 圖 12.1(b) 顯示每個類別的兩條密度曲線
    - 非 STEM 簡歷傾向使用較少標點符號
    - 對於使用約 150 個標點符號的個人：STEM 簡歷密度為 0.697，非 STEM 為 0.558

- **整體預測計算過程**：
  1. 將每個類別的各預測變數值相乘，形成**似然度統計量**（likelihood statistic）
  2. 表 12.1 顯示計算詳情
  3. 宗教信仰和標點符號數據都更符合 STEM 簡歷
  4. 類別似然值比率為 $0.148 \div 0.057 = 2.59$，表明該簡歷更可能對應於 STEM 職業
  5. 但貝葉斯規則還包括先驗信息項（STEM 簡歷比率約為 18.5%）
  6. 最終預測：將先驗和似然度相乘，值標準化為總和為 1
  7. 結果：該特定簡歷與 STEM 工作相關的概率僅為 37%

- **單純貝葉斯模型的特點**：
  * 優點：
    - 分別考慮每個預測變數使計算更快
    - 模型可以輕鬆部署，計算似然度所需的信息可以存儲在高效查找表中
  * 局限性：
    - 獨立預測變數的假設對許多數據集來說不合理
    - 例如：標點符號數量與字數有強烈的等級相關性（相關性：0.847）
    - 但即使忽略預測變數之間的相關性，模型仍可能產生良好預測

- **質性預測變數處理**：
  * 宗教信仰未被分解為指標變數
  * 另一種方法：可將此預測變數分解為所有 10 個宗教回應的完整集合
  * 需注意：新的二元變數仍應被視為質性變數
  * 否則，使用連續密度函數表示只有兩個可能值的預測變數
  * 性能比較：
    - 使用所有特徵集（約 110 個變數）的模型：ROC 曲線下面積為 0.798
    - 將質性變數轉換為單獨指標時：ROC 曲線下面積為 0.799
    - 性能差異可忽略不計，但個別指標變數的模型計算時間增加了 1.8 倍

- **減少預測變數的好處**：
  * 減少注入概率計算的噪音
  * 輔助好處：使用較少特徵時，獨立性假設產生的高度病理性類別概率分布問題減少
  * 當預測變數數量接近訓練集點數時，類別概率分布變得兩極化
  * 很少預測點落在範圍中間（大於 20% 且小於 80%）
  * 這些 U 形分布難以解釋，因為大多數錯誤預測的樣本有很高的確定性
  * 減少預測變數可以緩解此問題

- 本模型將用於演示使用 OkCupid 數據的**全局搜索方法**（global search methods）

### 注意：
- 先驗概率也在第 3.2.2 節中討論過，當時將其用於某些計算中的流行率

## 12.2 模擬退火（Simulated Annealing）

- **退火**（Annealing）是一種加熱金屬或玻璃的過程，用於去除瑕疵並提高材料強度
  * 當材料加熱時，粒子在材料內隨機快速重新排列，幫助加強弱分子連接
  * 材料冷卻時，隨機重新排列繼續進行，但速度較慢
  * 整個過程由時間和材料在特定時間的能量控制
...
- **模擬退火在特徵選擇中的應用**：
  ...
  * 類比：
    ...
    - 時間 = 迭代次數
    - 材料能量變化 = 預測性能變化
  * 此方法由 Kirkpatrick 等人（1983）和 Van Laarhoven 與 Aarts（1987）提出

- **模擬退火過程**：
  1. 隨機選擇初始特徵子集並確定迭代次數
  2. 建立模型並計算預測性能
  3. 隨機包含或排除當前特徵集中的一小部分（1-5%）特徵
  4. 計算新集合的預測性能
  5. 如果性能改善，保留新特徵集
  6. 如果性能下降，計算接受概率：$Pr[accept] = \exp[-ic(\frac{old-new}{old})]$
     * $c$ 是控制特徵擾動率的常數，默認為 1
  7. 生成隨機均勻數字，如果大於接受概率，拒絕新特徵集
  8. 這種隨機性使算法能夠跳出局部最優解

- **接受概率示例**：
  * 假設目標是優化預測準確度
  * 先前解決方案：85% 準確度，當前解決方案：80% 準確度
  * 相對下降比例：0.059
  * 如果在第一次迭代發生，接受概率為 0.94
  * 如果在第五次迭代發生，接受概率為 0.75%
  * 如果在第五十次迭代發生，接受概率降至 0.05
  * 接受較差解決方案的概率隨著算法迭代次數增加而下降

- **重啟機制**（restarts）：
  * 提供額外保護，避免在不利區域停留
  * 如果在 $I$ 次迭代內未找到新的最優解，搜索重置為最後已知的最優解

### 12.2.1 避免過度擬合的特徵選擇（Selecting Features without Overfitting）

- **如何使用數據**：
  * 如第 10.4 節所述，同一數據不應同時用於選擇、建模和評估
  * 應使用外部重採樣程序確定搜索的適當迭代次數
  * 迭代次數實際上是一個調整參數

- **雙層重採樣方法**：
  * 外部重採樣：例如 10 折交叉驗證，將模擬退火應用於 90% 數據
  * 內部重採樣：將外部分析集進一步分割為建模子集（80%）和評估子集（20%）
  * 目的：
    - 內部重採樣：引導子集選擇過程
    - 外部重採樣：幫助調整搜索迭代次數
  * 圖 12.2 說明了外部和內部交叉驗證方法

- **流程摘要**：
  1. 完成外部重採樣後，確定最佳搜索迭代次數
  2. 在整個訓練集上進行最終模擬退火搜索
  3. 確定最佳預測變數集
  4. 使用最佳預測變數集建立最終模型

### 12.2.2 應用於 OkCupid 數據建模（Application to Modeling the OkCupid Data）

- **數據描述**：
  * 訓練集：38,809 個簡歷，包括 7,167 個 STEM 領域簡歷
  * 使用 10 折交叉驗證作為外部重採樣
  * 驗證集：10% 的數據（646 個 STEM 簡歷和 2,848 個其他簡歷）
...
- **內部 ROC 估計**（圖 12.3）：
  * 各外部重採樣間趨勢有所不同
  * 一些顯示性能隨迭代增加，其他變化很小
  * 即使使用近 35,000 個數據點，選擇過程仍有巨大變異

- **內部與外部性能一致性**：
  * 兩組 ROC 統計之間的平均等級相關為 0.622，範圍為 (0.43, 0.85)
  * 表示相當好的一致性
  * 最佳性能與 383 次迭代相關
  * 最佳迭代次數範圍：69 到 404，平均 258
  * 選定變數數量更精確：平均 56 個，範圍 (49, 64)

- **最終搜索結果**：
  * 使用 10% 驗證集和其餘訓練數據
  * 隨著迭代次數增加，子集大小下降，ROC 曲線下面積緩慢上升
  * 最佳迭代（356）：ROC 曲線下面積為 0.837，使用 63 個預測變數
  * 最終選擇 66 個預測變數（共 110 個），詳細列表於原文
  * 基於外部重採樣的性能估計：ROC AUC 為 0.799
  * 略優於第 12.1 節的完整模型，主要好處是減少預測變數數量

- **隨機化驗證**：
  * 生成 100 個大小為 66 的隨機子集
  * 模擬退火模型優於 95% 的隨機子集
  * 表明此方法能找到良好的預測子集

### 12.2.3 性能變化檢驗（Examining Changes in Performance）

- **後續分析**：
  * 模擬退火是一種受控隨機搜索
  * 可以創建數據集量化有無每個預測變數的性能差異
  * 最簡單的分析是基本 t 檢驗
  * 最重要的 10 個預測變數（按絕對 t 統計量排序）：
    - 教育水平（education level）
    - 軟件（software）
    - 工程師關鍵詞（engineer keyword）
    - 收入（income）
    - 字數（the number of words）
    - 創業（startup）
    - 解決（solving）
    - 每字字符數（the number of characters/word）
    - 身高（height）
    - 小寫空格字符數（the number of lower space characters）
  * 這些預測變數中許多相關，測量與文章長度相關的特徵

### 12.2.4 分組定性預測變數與指標變數比較（Grouped Qualitative Predictors Versus Indicator Variables）

- **比較方法**：
  * 將分類變數轉換為虛擬變數並進行相同分析
  * 初始模型使用一半預測變數集
  * 由於預測變數從 110 增加到 298，迭代次數從 500 增加到 1,000
  * 性能結果：
    - 保持類別在單一預測變數中：AUC 0.799
    - 分解為個別預測變數：AUC 0.801（略差）

### 12.2.5 初始子集的影響（The Effect of the Initial Subset）

- **實驗設置**：
  * 使用不同初始子集大小（總特徵集的 10%、50% 或 90%）
  * 圖 12.7 顯示每個外部重採樣的平滑子集大小
...
## 12.3 遺傳演算法（Genetic Algorithms）

- **遺傳演算法**（Genetic Algorithm, GA）是基於進化族群生物學概念的優化工具（Mitchell 1998；Haupt, Haupt, 和 Haupt 1998）
  * 已被證明能夠找到複雜函數的最優或近似最優解（Mandal, Jeff Wu, 和 Johnson 2006）
  * 通過模仿族群進化過程尋找最優解：
    - 生成候選解決方案
    - 允許解決方案繁殖並創造新解決方案（繁殖）
    - 促進競爭，使最適合的解決方案（最優）有最佳機會存活並繁衍下一代（自然選擇）
  * 這個過程使遺傳演算法能隨時間優化解決方案，並已被證明會收斂到優化平台（Holland 1992）

- **生物學類比與特徵選擇**：
  * 在生物學中，每個族群中的個體由染色體（chromosome）表示，由基因（gene）序列構成
  * 每個染色體基於適應度標準（fitness criterion）評估
  * 適應度越高，染色體越有可能被選中用於繁殖下一代
  * 繁殖階段包括交叉（crossover）和突變（mutation）過程
  * 特徵選擇應用：
    - 族群 = 特定資料集所有可能的特徵組合
    - 染色體 = 特定特徵組合（即基因）
    - 適應度標準 = 所選特徵組合的預測能力
...
- **遺傳演算法實施**：
  * 初始族群需足夠大以包含特徵子集空間中的充分多樣性
  * 通常選擇 50 個或更多隨機選定的特徵子集作為初始生成
  * 使用包含 10% 至 90% 總特徵數的隨機子集創建初始族群
  * 評估每個特徵子集的適應度（預測能力）

- **玩具示例**：
  * 九個預測變數（A 至 I）和 12 個初始特徵子集
  * 每個子集有相關性能值（數值越大越好）
  * 父本選擇：使用加權隨機樣本，選擇概率為預測性能函數
  * 交叉過程：
    - 在兩個預測變數之間隨機選擇一個位置（單點交叉）
    - 父本交換交叉點一側的特徵
  * 突變過程：
    - 對每個可能的特徵生成小隨機概率（1%-2%）
    - 如果特徵在突變過程中被選中，則其狀態在當前特徵子集中改變

- **精英主義**（Elitism）：
  * 繁殖過程不保證當前世代中最適合的染色體會出現在下一代
  * 精英主義：將最佳染色體原封不動地帶入下一代
  * 優點：確保後續世代保持相同適應度水平
  * 缺點：可能導致算法在局部最優解中徘徊更長時間

### 12.3.1 外部驗證（External Validation）

- **與模擬退火類似**，應使用外部重採樣程序確定適當的迭代次數
  * 世代數是一個調整參數
  * 在每個外部重採樣循環內執行遺傳演算法
  * 使用內部重採樣測量適應度，確保建模和評估使用不同數據
  * 這有助於防止遺傳演算法找到過度擬合可用數據的特徵子集
...
- **關鍵觀察指標**：
  * 特徵子集大小和預測性能估計隨世代變化
  * **Jaccard 相似度**（Tan, Steinbach, 和 Kumar 2006）：
    - 衡量子集間的多樣性
    - 計算為兩個子集共有預測變數數量與兩者唯一預測變數總數的比例
    - 例如：五個可能預測變數中，子集 ABC 和 ABE 的 Jaccard 相似度為 2/4 或 50%
    - 提供演算法如何有效地收斂到共同解決方案或潛在過度擬合的見解

- **結果分析**（圖 12.8）：
  * 儘管重採樣在第一代大小上有相當變化，但都收斂到包含 61 至 85 個預測變數的子集大小
  * 相似度圖表明世代內的子集越來越類似於最佳解決方案
  * ROC 曲線下面積開始平穩到平均值 0.843
  * 由於 ROC 曲線下平均面積整體呈上升趨勢，數值最大的世代為 15
  * 趨勢仍在繼續，額外世代可能在後續世代產生更優化的特徵子集

- **最終搜索**：
  * 在整個訓練集上進行
  * 結果與先前內部趨勢非常相似（圖 12.9）
  * 第 15 代的最終預測變數集包含比先前 SA 搜索更多的變數，詳細列表於原文
  * 為評估搜索有效性，選擇 100 個大小為 63 的隨機子集
  * GA 選擇的子集性能優於 100% 的隨機選擇子集
  * 表明 GA 確實找到了用於預測響應的有用子集

### 12.3.2 強制稀疏性（Coercing Sparsity）

- **遺傳演算法的特點**：
  * 往往比其他方法選擇更大的特徵子集
  * 可能原因：如果特徵對預測有用，則會被包含在特徵子集中
  * 保留對預測性能沒有影響的特徵的懲罰較小
  * 尤其在與不會對包含不相關預測變數產生實質性懲罰的建模技術結合時（第 10.3 節）

- **稀疏解決方案的好處**：
  * 模型通常更簡單，更容易理解
  * 更少的預測變數需要更少的計算資源
  * 問題：如何強制搜索程序朝向更稀疏的預測變數子集解決方案？

- **方法：使用懲罰機制**
  * 使用帶有基於特徵數量明確懲罰的性能代理測量
  * 第 11.4 節介紹了**赤池信息準則**（AIC），它使用訓練集大小和模型項數的函數增強目標函數
  * AIC 專為使用似然函數作為目標的模型量身定制，但不能應用於優化其他指標（如 ROC 曲線下面積或 $R^2$）
  * AIC 通常涉及模型參數數量而非預測變數數量
  * 許多模型沒有參數數量的概念（如樹，$K$-NN 等）

- **多參數優化**（Multi-Parameter Optimization, MPO）：
  * 基於懲罰性能測量使問題能夠以多參數優化框架呈現
  * MPO 使用函數將多個目標值組合為最終單一值
  * 特徵選擇目標：
    1. 優化預測性能
    2. 包含盡可能少的特徵
  * **期望函數**（Desirability Functions）方法（Harrington 1965；Del Castillo, Montgomery, 和 McCarville 1996；Mandal 等人 2007）：
    - 每個目標轉換到新尺度 [0, 1]，較大值更期望
    - 範例：ROC 曲線下面積 0.50 不期望，1.0 期望
    - 簡單期望函數：當 AUC≤0.50 時為零，AUC=1.0 時為 1.0
    - 模型中預測變數數量的期望函數：所有預測變數都在模型中時為 0.0，只有一個預測變數時為 1.0

- **整體期望統計量計算**：
  * 通過取所有個別函數的幾何平均值創建
  * 如果有 $q$ 個期望函數 $d_i(i=1…q)$，幾何平均值定義為：
  
$$D = \left(\prod_{i=1}^q d_j\right)^{1/q}$$

  * 基於此函數，如果任何一個期望函數不可接受（即 $d_i=0$），整體測量也不可接受
  * 對於特徵選擇應用，GA 使用的內部性能指標可以使用整體期望來鼓勵選擇過程偏向較小子集

- **常用期望函數**：
  * Derringer和Suich（1980）提出的分段線性或多項式函數最為常用
  * 最大化數量的函數：
...
\end{cases}$$
  ...
  * 圖 12.10(a) 顯示最大化函數示例，$A=0.50$，$B=1.00$
  * $s<1.0$ 削弱期望函數影響，$s>1.0$ 增強期望函數影響
  * 圖 12.10(b) 顯示使用幾何平均結合兩個函數的結果

- **OkCupid 數據應用期望函數**：
  * 目標：
    1. 最大化 ROC 曲線下面積，$A=0.50$，$B=1.00$，$s=2.0$
    2. 最小化子集大小，$A=10$，$B=100$，$s=1.0$
  * 使用整體期望作為內部性能，外部 ROC 曲線下面積仍作為主要外部指標
  * 結果（圖 12.11）：
    - 整體期望持續增加
    - 平均子集大小減少到最後世代約 28 個
    - 第 14 代具有最大期望值，外部 ROC 曲線下面積估計為 0.802
    - 低於未限制的 GA，但效能損失由模型中預測變數數量的顯著減少所抵消
  * 最終遺傳演算法運行在第 15 代選擇了 32 個預測變數，詳細列表於原文

- **選擇考量**：
  * 即使結果略差（但優於完整模型），較小的預測變數集也有潛在優勢
  * 如果單純貝葉斯優於其他模型，值得在測試集上評估正常和稀疏遺傳演算法

### 注意：
- 還有多點交叉方法，可創建超過兩個交換特徵的區域
- 通常還會生成隨機概率來確定是否應進行交叉
- 遺傳演算法的世代數通常在 100 到 1000 之間，但如此大的值會使特徵選擇過程在內部和外部驗證方案下計算上具有挑戰性
- 如果不希望由於只有一個不理想特性而將期望分數強制為零，則可以限制個別期望函數使最小值大於零

## 12.4 測試集結果（Test Set Results）

- 使用**最佳參數設置**（optimal parameter settings）建立了兩種模型：
  * **無約束遺傳演算法**（unconstrained GA）
  * **有約束遺傳演算法**（constrained GA）

- **測試集性能比較**：
  * 無約束 GA 的 ROC 曲線下面積（area under the ROC curve）為 0.831
  * 有約束 GA 的 ROC 曲線下面積為 0.804
  * 這兩個值均高於其類似的重採樣估計值（resampled estimates）
  * 兩個模型的性能排名順序相同

- **性能差異**（如圖 12.12 所示）：
  * 無約束模型在整個測試集截斷值（test set cutoffs）範圍內具有一致更好的**敏感度**（sensitivity）和**特異度**（specificity）
  * 然而，很難判斷這些結果是否存在顯著差異

- **結論含義**：
  * 無約束 GA 提供略高的預測性能
  * 有約束 GA 提供更精簡的模型，但犧牲了少量性能
  * 在實際應用中，可能需要在模型複雜性與預測能力之間取得平衡
  * 選擇哪種模型取決於特定應用場景的需求和優先事項

## 12.5 總結（Summary）

- **全局搜索方法**（global search methods）是調查預測變數空間並識別與響應變數最佳相關的預測變數子集的有效工具
  * 儘管這些方法有一些可調參數（如迭代次數、初始子集大小、突變率等）
  * 默認值通常合理且效果良好

- **計算挑戰**：
  * 全局搜索方法雖然通常有效，但計算成本很高
  * 例如：
    - 完整的模擬退火（simulated annealing）分析過程需擬合共 5,501 個獨立模型
    - 遺傳演算法（genetic algorithm）分析需要 8,251 個模型
  * 外部交叉驗證（external cross-validation）折疊的獨立性允許這些方法並行運行
    - 減少整體計算時間
    - 即使如此，使用並行處理的遺傳演算法在每個單獨核心上運行外部交叉驗證折疊時仍需超過 9 小時

- **模型複雜性的影響**：
  * 本章使用的單純貝葉斯模型（naive Bayes model）沒有優化任何調整參數
  * 若全局搜索方法與具有調整參數的模型結合：
    - 徑向基支持向量機（radial basis support vector machine）（2 個調整參數）
    - C5.0 模型（3 個調整參數）
  * 計算需求將顯著增加

- **實際應用建議**：
  * 當結合具有調整參數的模型使用全局搜索方法時：
    1. 首先使用問題相關的專家知識來縮減特徵集
    2. 識別合理範圍的調整參數值
    3. 如果有足夠樣本可用，可以分離一部分來尋找使用全部特徵的潛在良好參數值範圍
    4. 調整參數值可能不是特徵子集的完美選擇，但應足以有效找到最佳子集

- **注意事項**：
  * 核密度估計（kernel density estimates）的平滑度可能被調整，但通常不會對性能產生有意義的影響